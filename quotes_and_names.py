...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  9 20:49:31 2019

@author: jhutt5383
"""

#Quotes and names for Futurama gmae
...
#I have been entering quotes manually - this method checks to make sure there are no type-os missing "" before running the game
def testData():
    """Input: Uses global variables quotes and names
    Output: prints error messages if data entries do not correspond to their appropriate type"""
    
    errors = False
    for i in range(len(quotes)):
        if type(quotes[i][0]) != type('a'):
            print('Error with Quotes at index ' + str(i))
            errors = True
        if type(quotes[i][1]) != type('a'):
            print('Error with Quotes - name at index ' + str(i))
            errors = True
        if type(quotes[i][2]) != type(1):
            print('Error with Quotes - point value at index ' + str(i))
            errors = True
            
    for j in range(len(names)):
        if type(names[j]) != type('a'):
            print('Error with names at index ' + str(j))
            errors = True
